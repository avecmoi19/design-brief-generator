...
function buildPrompt({ season, chapter, item, material, extra }) {
  const matLine = material
    ? `- 중심 소재: ${material}  (이번 브리프는 이 소재를 축으로 한 시리즈 개발 관점에서 작성하라)`
    : `- 중심 소재: 미지정`;
  return `너는 여성복 브랜드의 시니어 상품기획 디렉터다. 아래 브랜드와 조건에 맞는 시즌 디자인 브리프를 작성하라.
...
module.exports = async (req, res) => {
  ...
  if (!key) {
    res.status(500).json({ error: "서버에 ANTHROPIC_API_KEY가 설정되지 않았어요. Vercel 환경변수를 확인하세요." });
    return;
  }
  ...
  try {
    ...
    const { season, chapter, item, material, extra } = body;
    if (!season || !item) {
      res.status(400).json({ error: "시즌과 아이템은 필수예요." });
      return;
    }
    ...
    const r = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        model,
        max_tokens: 2000,
        messages: [{ role: "user", content: buildPrompt({ season, chapter, item, material, extra }) }],
      }),
    });

    if (!r.ok) {
      const detail = await r.text();
      res.status(502).json({ error: "생성 요청이 실패했어요.", detail });
      return;
    }
